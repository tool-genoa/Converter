"""
  It contains three classes: Species, Reaction, and Kinetic. 
...
from parameters import n_soap_group, BaseSpecies, psat_svoc, psat_nvoc, vptype
...
class Species:
    ...
    def __init__(self, name=None, strin=None, mass=0.0):
        ...
        self.functionalGroups = (
            {}
        )  # Functional group info (used to define lumpable species)
        self.SOAPStructure = (
            None  # No.non-zero function groups in SSH-aerosol vector format
        )
    ...
    def write_to_text(self, attrs=species_attr):
        ...
        return "\n".join(
            [f"{s}\t{json.dumps(attr_vals[s])}" for s in attr_vals if attr_vals[s]]
        )
    ...
    def update_with_dict(self, new_dict, check=True):
        ...
        # Update psat_torr
        if self.condensable:
            self.psat_torr = self.psat_atm * 760.0

        # Update ratios and DU
        val = {i: self.functionalGroups[i] for i in ["C", "H", "N", "O"]}
        self.ratios["OM/OC"] = round(self.mass / (val["C"] * 12.0), 3)
    ...
    def update_based_on_smiles(self):
        ...
        if self.SMILES == "":
            raise ValueError(f"Read non SMILES for species {self.name}")
        elif self.SMILES == "-":
            raise ValueError(
                f"Species Update: No SMILES for the given species: {self.name}"
            )
        ...
        self.psat_torr = self.psat_atm * 760.0  # unit conversion
        self.dHvap_KJ = (
            mp.get_enthalpy_vaporization(pymol, vptype) / 1000.0
        )  # unit in KJ

        # Check for non-volatility
        if psat_nvoc and self.psat_atm < psat_nvoc:
            print(
                f"species {self.name} is set to non_volatile, with P_sat = {self.psat_atm} < {psat_nvoc}"
            )
            self.non_volatile = 1
        ...
        if 0:
            ...
            if hasattr(self, "gamma") and self.gamma:
                ...
                self.henry = 1000 / (
                    18 * self.gamma * self.psat_atm
                )  # Henry's law constant
                self.Kp = mp.get_partitioning_coefficient(
                    self, temp
    ...
    def update_flags(self):
        ...
        if "[O+]" in self.SMILES or "[O]" in self.SMILES or "O." in self.string:
            ...
            if any(i in self.SMILES for i in ["O[O]", "[O]O"]) or "OO." in self.string:
                self.RO2 = 1  # Default Pool 1
    ...
    def update_condensable(self, Psat_condense=psat_svoc):
        ...
        if not self.Radical and (
            self.psat_atm != 0.0 and self.psat_atm <= Psat_condense
        ):
        ...
        if self.Radical:  # Radical
            ...
            if mode == "none":
                ...
                if self.SOAPStructure != None:
                    itype += " ".join(
                        [
                            f"{self.SOAPStructure[i]}:.1f"
                            for i in sorted(self.SOAPStructure.keys())
                        ]
                    )
            else:
    ...
    def SOAPStructure_to_ssh_str(self):
        ...
        for i in range(n_soap_group):
            j = str(i)
            if j in self.SOAPStructure:
                num = self.SOAPStructure[j]
            else:
                num = 0.0
    ...
    def to_ssh_aerosol_list(self, mode="smiles", firstRun=False):
        ...
        # Structure
        if mode == "vectors":
            struc = self.SOAPStructure_to_ssh_str()
        elif mode == "smiles":
            struc = self.SMILES
        else:
    ...
    def gecko_str_to_smiles(self, tag_canonical=True):
        ...
        if self.string == None:
            print(
                f"MD: GECKO structure is None for {self.name}, cannot be converted to SMILES."
            )
            return False
    ...
    def get_ratios(self, slist, update_rate=False, rate_inds=[0], get_fnc=False):
        ...
        if update_rate:
            ...
            for i, s in enumerate(self.rate.values):
                if i in rate_inds and s > 0.0:
                    rates[i] = s
        else:
            rates = np.ones(nr)  # No kinetic rate as a value

        # Get basic ratios
        for i, s in enumerate(slist):
            if s in self.products:
                for j in range(nr):
                    ratios[i][j] = self.products[s] * rates[j]
        ...
        if self.rt_fnc is not None:
            for i, s in enumerate(slist):
                if s in self.rt_fnc:
                    ...
                    if get_fnc:
                        fnc[s] = [rt_rate[j] for j in rt_inds]
                    else:
                        for j in range(nr):
                            irt = 0.0
        ...
        if nr == 1:
            ratios = ratios.reshape(-1)  # reshape nr if need
        ...
        if self.rt_fnc is None:
            for s in sorted(self.products.keys()):
                if self.products[s] != 1.0:
                    parts.append(f"{self.products[s]:6.3E} {s}")
                else:
                    parts.append(s)
            ...
            for s in sorted(self.products.keys()):
                rt = self.products[s]
                if (
                    s in self.rt_fnc and s not in BaseSpecies
                ):  # in format speies: k1, k2
                    ...
                    if len(tmp) % 2 == 0:  # Check length
                        ...
                        raise ValueError(
                            f"Not recognize function ratio for {s}: {tmp}", outline
                        )
                # No function
                elif rt == 1.0:
                    parts.append(s)
                else:
                    parts.append(f"{rt:6.3E} {s}")
    ...
    def from_line_wo_kinetic(self, line, separator="->"):
        ...
        for i, s in enumerate(parts + ["+"]):  # Add + to find end
            if s == "+":  # Find end
                ...
                if n1 not in [1, 2, 4]:
                    raise ValueError(
                        f"Find {n1} elements for one product part: {sps} in {line}. n1 should be 1,2,4. parts: {parts}",
                        i,
                        s,
                    )
                ...
                if n1 == 4:  # Find function ratio
                    if self.rt_fnc is None:
                        self.rt_fnc = {"rates": [], "values": []}
                    # with more than one fratio
                    if isp in self.products:
                        if rt != self.products[isp]:
                            raise ValueError(
                                f"Find different ratio for {isp}: {rt} and {self.products[isp]} in {line}"
                            )
                    else:
                        self.products[isp] = rt  # Add ratio
                    # print(f'Find function ratio for {isp}: {sps} in {line}')

                    if isp not in self.rt_fnc:
                        self.rt_fnc[isp] = []
                ...
                sps.append(s)  # Find one element: ratio or species
        return
        ...
        for line in strin.split("\n"):
            ...
            if line.startswith("%"):  # Find comments
                ...
                if n > 1:  # Find for ratio
                    if self.rt_fnc is None:
                        raise ValueError(
                            f"Find multiple kinetic lines but the no sign for function ratio.",
                            strin,
                        )
                    else:  # Update rate
...
class Kinetic:
    ...
    def __init__(self, strin=None, sshin=None):
        ...
        self.values = (
            None  # Kinetic rate computed based on current conditions (env_params)
        )
    ...
    def update_flags_from_ssh(self):
        ...
        # Check Photolysis
        if "PHOT" in self.ssh or "EXTRA 91" in self.ssh or "J<" in self.str:
            self.Photolysis = True
        ...
        if "\tlRs:" in self.str:
            self.ratios.extend(
                [
                    float(i)
                    for i in self.str.split("\tlRs:")[1].split("\t")[0].split("*")
                ]
            )
        ...
        if "\tK:" in self.str:
            self.values = [
                float(i) for i in self.str.split("\tK:")[1].split("\t")[0].split(",")
            ]
    ...
    def update_by_ratio(self, ratio, mode="add"):
        ...
        if mode == "add":
            if self.values:
                self.values = [i * ratio for i in self.values]
            self.ratios.append(ratio)
        elif mode == "new":
            if self.values:
                self.values = [ratio for i in self.values]
            self.ratios = [ratio]
        else:
    ...
    def update_str(self):
        ...
        if self.values:
            k_str = "K:{','.join([f'{i:6.3E}' for i in self.values])}"
        else:
            k_str = ""
        new_str = ""
    ...
    def update_ssh_by_ratio(self, ratio, mode="add"):
        ...
        if ratio == 1.0:
            ...
            raise ValueError(
                f"Ratio {ratio} < 0.0 for reaction with kientic: {self.ssh}"
            )
        ...
        if label == "ARR":
            ind = parts.index(label) + 1
        # Check label for those with ratio at the tail
        elif label in ["PHOT", "PHOTOLYSIS", "EXTRA", "FALLOFF"]:
            ind = -1

        if mode == "add":
            parts[ind] = f"{float(parts[ind])*ratio:6.3E}"
        elif mode == "new":
            parts[ind] = f"{ratio:6.3E}"
        else:
            raise ValueError(f"Not recognize mode {mode}")
    ...
    def get_kinetic_info(self):
        ...
        if parts[ilabel] == "ARR":  # ARR + nums
            detailed_keys.append(
                " ".join(
                    ["ARR"] + [format(float(i), ".2e") for i in parts[ilabel + 2 :]]
                )
            )
        ...
        # Check label for those with ratio at the tail
            ...
            detailed_keys.append(
                " ".join(
                    [parts[ilabel]]
                    + [format(float(i), ".2e") for i in parts[ilabel + 1 : -1]]
                )
            )  # Remove ratio
            ratio = float(parts[-1])  # ratio
        else:
            raise ValueError(f"Unknown keyword: ", ilabel, " ".join(parts))
        # Output
