...
from umansysprop import groups, boiling_points, vapour_pressures, activity_coefficient_models
...
def get_pybelmol(mol):
    ...
    # if input a SMILES string
        ...
        print(mol) 
        raise ValueError('get_pybelmol: not recognize data type, not mol and not string.', mol)
...
def get_activity_coefficient_inf(compound,temperature=298.):
...
def get_activity_coefficient_org(compounds,concs,temperature=298.):
...
def get_partitioning_coefficient(compund, temperature=298., Mavg=200):
    ...
    for i in SOAPStructure:
        ...
        for k in ksim:
            if i in simpol0[k]:
                ...
                tag = 1
                break
        if tag == 0: print('!!!!not find strucutre: ', i)
...
def mechgen_to_smiles(mechgen,tag_canonical=True):
    ...
    stars = [i for i in range(len(mechgen)-1) if (mechgen[i] == "*" and not isfloat(mechgen[i+1]))]
        ...
        for i,s in enumerate(stars):
            ...
            for j in range(len(subline)-1,-1,-1): # reverse
                if subline[j] == "C": 
                    ...
                    break 
            # check if find C
            if ind == -1: raise ValueError("Not found C in the subline: ",subline,i,s,mechgen)
...
if __name__ == '__main__':
    ...
    if 0: # test Psat calculation
        ...
        for mode in ['VP0BP0', 'VP0BP1', 'VP0BP2', 'VP1BP0', 'VP1BP1', 'VP1BP2', 'simpol', 'evap', 'evap2']:
    ...
    if 0: # test MechGen to SMILES
        mol = ['CH2=C*1-CH*2-CH2-CH2-CH(CH2*2)-C*1(CH3)-CH3','CH3-C(CH3)(CO-CH2-OH)-C(ONO2)(CH2-CH2-O-OH)-CH2-CO-O-OH']
        for i in mol:
            print(f'From MechGen format: {i}\n To SMILES format: {mechgen_to_smiles(i)}\n')
