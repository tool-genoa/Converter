"""
  It converts the knietic rate contants from MCM format to a format can be executed by SSH-aerosol. 
    ...
    for key in mcm_manual_lists:
        if key in kin:  # Found pair
            ...
            if key == kin:
                ...
                try:  # Update factor - 1st case
                    ...
                    try:  # Update factor - 2nd case
                        ...
                        print(
                            f"MCM maunal can not process rate sting: {kin} with key: {key} and res: {res}"
                        )
                        return None
            # Process factor
            operator = res[0]
            if operator == "*":
                ...
                raise ValueError(f"Found unrecognized operator {operator} in {kin}")

            if "EXTRA 99" in val:
                return f"KINETIC {val} {ratio:6.3E}"  # Add a ratio
                ...
                raise ValueError(
                    f"Need to manually update the rate {val} with factor {factor} in {kin}"
                )
...
def mcm_to_ssh_photolysis(Jin):
    ...
    if ind.isdigit():
        ...
        if tmp:
            return (
                f"KINETIC EXTRA 91 {tmp[0]} {tmp[1]:5.3f} {tmp[2]:5.3f} {factor:6.3E}"
            )
        else:
...
def mcm_to_ssh_kinetic_rate(kin):
    ...
    for val in klist:
        ...
        for key, value in tmp.items():
            ...
            # Update unknown
                ...
                df[key] = "\t".join([df[key], tmp[key]])
            else:
                print(
                    f"Found multple values for the same key {key} that can not be merged: {df}, {tmp}, from {kin}"
                )
    ...
    # MCM specific rates
    if "GRC" in df:
        return rate_ssh + f"EXTRA 92 {mcm_simple_rate_coefs[df['GRC']]} {C1}"
    if "CRC" in df:
        return rate_ssh + f"EXTRA 93 {mcm_complex_rate_coefs[df['CRC']]} {C1}"
...
def gecko_to_ssh_kinetic_rate(rate_str):
    ...
    if rate_type == "HV":  # no arrhenius(3) is used
        ...
        if len(rate_info) != 2:
            raise ValueError(
                f"Invalid parameters for 'HV' type: {rate_info}",
                ind0,
                fin[ind0 : ind0 + 2],
            )
    ...
    # FALL OFF with 11 paramters
        ...
        rate_ssh += " ".join(map(str, arrhenius + rate_info[1:] + [1.0]))
