...
def to_SSH(compound):
    ...
    # match results to SSH-aerosol index
    ...
    m1 = aiomfac(compound)
    uman_ssh = { # index in result to index in SSH-aerosol format
    #    1:0,2:1,3:2,4:3, # group C
        149:4,150:5,151:6,152:7, # group C[OH]
        141:8,142:9,143:10,144:11, # group Calcohol
        145:12,146:13,147:14,148:15, # group Calcohol-tail
    #    5:16,6:17,7:18,8:19,70:20, # group C=C
    #	0.0,0.0, //group aromatic carbon (AC)
    #	0.0,0.0,0.0, // group //AC-C
    #    14:26, # group OH
    #    16:27, # group H2O
    #    17:28, # group ACOH
    #    18:29,19:30, # group ketone
    #    20:31, # group aldehyde  
    #	0.0,0.0, //group ester
    #	0.0,0.0,0.0, //group ether 
    #    42:37, # group acid
    #	0.0,   //group ACNO2
    ...
    # get m
    m = matches(data.SSHAEROSOL_SMARTS, compound)
    m[55] = m[5501] + m[5502]
    
    # transfer m1 to m
    for i in list(uman_ssh.keys()):
        ind = uman_ssh[i]
        m[ind+1] = m1[i] # m index from 1
