...
import Module as md
from Convert import isfloat, BaseSpecies, species_list_aer_init, n_soap_group, BaseSpeciesDict
from KineticRateToSSH import mcm_to_ssh_kinetic_rate, gecko_to_ssh_kinetic_rate
...
def process_gecko_reactions_and_species(geckopath, pvapfile='pvap.nan.dat', remove_empty_pero=True):
    ...
    for s in BaseSpecies:
        ...
        species.append(md.Species(name=s, mass=BaseSpeciesDict[s]))
...
def process_ssh_species(species_file, check_exist=None):
    ...
    for sp in BaseSpecies:
        if sp not in inds:
            species.append(md.Species(name=sp, mass=BaseSpeciesDict[sp]))
